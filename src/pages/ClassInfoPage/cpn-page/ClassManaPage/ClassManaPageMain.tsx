...
export const ClassManaMain: React.FC<{ classList: ClassList[] }> = ({classList}) => {
  ...
  // 添加班级
  const AddClass = () => { setadd(false),Add(input) }
  ...
  return (
    <>
      <>
        ...
        {show && <Modal
          title={
            renameState ?  <BaseLoading />  : <Typography.Title
              editable={{
                onChange:(value)=> {setInput(value)},
                onEnd() {Rename({classId:show.class_id,className:input}),setVis(false)},
              }}
              level={4}
              style={{ margin: 0 }}
            > {`${show.class_name}`} </Typography.Title>
          }
          centered
          visible={vis}
          width='1000px'
        ...
        {/* 主体内容 */}
              ...
              tool={
                <PrimaryButton handleClick={()=>setadd(true)} title="新建班级" />
              }></GlobalHeader>
              <GlobalRightLayout>
                <Row gutter={[16, 24]}>
                  {classList!.map(i => (
